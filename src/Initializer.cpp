...
void Initializer::InitializeAngle_Azimuthal(vector<Particle> & pList) {
	vector <Particle>::iterator it;
	for (it=pList.begin();it!=pList.end();it++)
		{ double angle=it->getPos().angle()+0.5*pi;
...
void Initializer::update_par(vector<Particle> & pList, Box * & box, Box * & inibox) {
	par->np=pList.size();
	box->updatePar(par);
	inibox->updateIniPar(par);
	double area=0.;
	for (int i=0;i<par->np;i++)
		{ area+=pi*pList[i].getRadius()*pList[i].getRadius(); }
...
// In 'N' mode, the phi is not exactly the one required because you can't add a fraction of a particle !
	if ((par->fillmode)=="N")
		{ validMode=true; par->np=(int) round(par->phi*inibox->getArea()/(pi*par->radius*par->radius));
...
// it could work the other way as well (could be made an option)
	...
	while (i<par->np)
		...
		if (inibox->isIn(pos))
			{
			pos+=par->offsetX*Geomvec2(1.,0.)+par->offsetY*Geomvec2(0.,1.);
...
// phi~np/L^2 where L=box size. 2 of these are fixed, the 3rd (=fillmode) is a function of them.
	if ((par->fillmode)=="N")
		{ par->np=(int) round(par->phi*inibox->getArea()/(pi*par->radius*par->radius));
...
// it could work the other way as well (could be made an option)
		box->rescale(a); box->printBox();
		if (par->iniboxtype!="Full") { inibox->rescale(a); inibox->printBox(); }
	...
	cout << par->BCtype << endl;
	
	if (par->BCtype=="Periodic")
		{
		double spacing=sqrt(2*pi/(sqrt(3)*par->phi)*par->radius);
		cout << "[Warning] 'Hex' initialization with periodic BC: box is assumed rectangular and fillmode=L." << endl;
		double ar=inibox->getBoundingSizeY()/inibox->getBoundingSizeX();	// aspect ratio
		cout << "aspect ratio: " << ar << endl;
		int nx=int(sqrt(par->np*sqrt(3)/(2*ar)));
		int ny=par->np/nx;
		cout << "nx=" << nx << " ; ny=" << ny << endl;
		
		double ax=nx*spacing/inibox->getBoundingSizeX();
		double ay=ny*spacing*sqrt(3)/2/inibox->getBoundingSizeY();
		inibox->rescale(ax,ay); inibox->printBox();
		if (par->iniboxtype!="Full") { box->rescale(ax,ay); box->printBox(); }
