...
// lines are x/Lx, y/Ly and 0.5*radius
		iss >> x >> y >> r;
		pList.push_back(Particle(i,Geomvec2(x*Lx,y*Ly),par));
...
void Reader::readPos(const string & filename) {
	...
	while (getline(is,line))
		...
		istringstream iss(line);
		iss >> i >> x >> y >> angle;
		pList.push_back(Particle(i,Geomvec2(x,y),angle*pi/180.,par));
		}
	is.close();
...
void Reader::readPosVel(const string & filename) {
	...
	while (getline(is,line))
		...
		istringstream iss(line);
		iss >> x >> y >> angle;
		pList.push_back(Particle(i,Geomvec2(x,y),angle,par));
		i++;
		}
	is.close();
	hasRadii=false; hasAngles=true;
...
//  cout << r2 << endl;
	...
	if (boundary.size()>5 && circle)
		...
		{ Geomvec2 u1=boundary[1]+boundary[2];
		Geomvec2 u2=boundary[2]+boundary[3];
		par->boxtype="Skew";
		par->boxVec1=u1;
		par->boxVec2=u2;
