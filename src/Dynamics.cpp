		...
		if (!(pList[i].getGlued()))
			...
			for (j=NL->getHNL(i).begin();j!=NL->getHNL(i).end();j++)
				...
				if (anginter->getSym())
					...
					{ double * torqueij=anginter->getTorqueNS(pList[i],*j);
					pList[i].addAngVel(torqueij[0]);
					j->getPart()->addAngVel(torqueij[1]); }
				}
			}
		}
}
...
//  targetAngle.resize(pList.size());		// only necessary if the number of particles changes
	...
	for (unsigned int i=0;i<pList.size();i++)
		{ Geomvec2 targetOrientation;
		if (!pList[i].getGlued())
			{ if (pList[i].getNc()!=0) { v[i]/=pList[i].getNc(); }
			targetAngle[i]=(par->a_vs*pList[i].getAVel()+par->a_vn*v[i]+par->a_F*pList[i].getVel()).angle(); }
		}
	cout << endl;
}
	...
	for (unsigned int i=0;i<pList.size();i++)
		...
		for (it=NL->getHNL(i).begin();it!=NL->getHNL(i).end();it++)
			...
			if (g || !(it->getPart()->getGlued()))
				{
				Geomvec2 forceij=inter->getForce(pList[i],*it);
...
void Dynamics_Euler_InstAng::setAngle(vector<Particle> & pList, NeighborList * NL) {
	for (unsigned int i=0;i<pList.size();i++)
		{
		pList[i].setAngVel(0.);
//    if (pList[i].getNc()!=0 && !pList[i].getGlued()) {
		if (!pList[i].getGlued()) {
			pList[i].setAngle(targetAngle[i]); }
//    cout << "nc=" << pList[i].getNc() << sep+"targetAngle=" << targetAngle[i] << endl;
		}
}
